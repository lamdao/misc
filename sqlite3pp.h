/*
  ...
  modified by: MrChuoi<at>gmail<dot>com
  date: 2015-08-19
 */
...
public:
	 ...
	 */
	explicit SQLite3PP(const std::string &filename)
	{
		if (sqlite3_open(filename.data(), &dbh) == SQLITE_OK
	...
	bool sql(const std::stringstream &query, const Args &... args)
	{
	...
	{
		sql_statement call;
		if (sqlite3_prepare_v2(dbh, query.data(), -1, &call.statement,
					nullptr) != SQLITE_OK)
		{
			return false;
		}
	...
	{
		sql_statement call;
		if (sqlite3_prepare_v2(dbh, query.data(), -1, &call.statement, nullptr) != SQLITE_OK)
			return false;
...
private:
	...
	struct bind_struct {
				...
				const T &first, const Args &... args)
		{
			std::stringstream ss;
			ss << first;
			if (sqlite3_bind_text(statement, current, ss.str().data(), ss.str().length(),
	...
	struct bind_struct<std::vector<char>, Args...> {
		...
		{
			if (sqlite3_bind_blob(statement, current, &first[0], first.size(),
						SQLITE_TRANSIENT) != SQLITE_OK)
			{
				return false;
			}
